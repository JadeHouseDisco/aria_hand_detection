...
class UDPSender:
    ...
    def send_data(self, skeleton_array, wrist_pos, wrist_quat, mode):
...
# --- LEFT HAND MODE DETECTOR ---
class LeftModeDetector:
    """
    Detects discrete control mode (0-5) from LEFT hand MediaPipe landmarks.
    ...
    Modes:
      ...
      4: index + middle + ring + pinky extended
      5: all fingers extended (above + thumb extended)
    """
    ...
    def __init__(self,
        ...
        self.loss_timeout_frames = loss_timeout_frames

    def _to_numpy(self, lm):
    ...
    def _is_finger_extended(self, pts, chain, wrist_idx=0):
        wrist = pts[wrist_idx]
        base = pts[chain[0]]
        pip  = pts[chain[1]]
        dip  = pts[chain[2]]
        ...
        cos12 = self._cos(v1, v2)
        cos23 = self._cos(v2, v3)
        straight = (cos12 > 0.85) and (cos23 > 0.85)

        dist_tip_wrist = np.linalg.norm(tip - wrist)
        dist_base_wrist = np.linalg.norm(base - wrist) + 1e-6
        far_enough = dist_tip_wrist > 1.1 * dist_base_wrist

        return straight and far_enough

    def _is_thumb_extended(self, pts, wrist_idx=0):
        wrist = pts[wrist_idx]
        thumb_tip = pts[4]
        index_tip = pts[8]

        v_wrist_thumb = thumb_tip - wrist
        v_wrist_index = index_tip - wrist

        cos_angle = self._cos(v_wrist_thumb, v_wrist_index)
        sideways_enough = cos_angle < 0.8

        dist_thumb = np.linalg.norm(thumb_tip - wrist)
        dist_index = np.linalg.norm(index_tip - wrist) + 1e-6
        long_enough = dist_thumb > 0.5 * dist_index

        return sideways_enough and long_enough

    def _classify_single_frame(self, mp_landmarks):
        pts = self._to_numpy(mp_landmarks)
        ...
        index_ext  = self._is_finger_extended(pts, FINGERS["index"])
        middle_ext = self._is_finger_extended(pts, FINGERS["middle"])
        ring_ext   = self._is_finger_extended(pts, FINGERS["ring"])
        pinky_ext  = self._is_finger_extended(pts, FINGERS["pinky"])
        thumb_ext  = self._is_thumb_extended(pts)

        finger_states = [index_ext, middle_ext, ring_ext, pinky_ext]
        ...
        for m, pattern in patterns.items():
            if all(fs == p for fs, p in zip(finger_states, pattern)):
                mode = m
                break

        if all(finger_states) and thumb_ext:
            mode = 5

        return mode

    def update(self, left_landmarks_list):
        """
        Call once per frame, even if there is no left hand.
        """
        self.frame_idx += 1

        if not left_landmarks_list:
            # No left hand this frame.
            if self.last_seen_frame_idx < 0:
                # Never seen a left hand: stay at default.
                self.current_mode = self.default_mode
                self.history.clear()
                return self.current_mode

            # Left hand disappeared: after some time, reset to default.
            if self.frame_idx - self.last_seen_frame_idx > self.loss_timeout_frames:
                if self.current_mode != self.default_mode:
                    self.current_mode = self.default_mode
                    self.history = [self.default_mode]
            return self.current_mode

        # Left hand visible → update from landmarks
        self.last_seen_frame_idx = self.frame_idx
        mode_now = self._classify_single_frame(left_landmarks_list[0].landmark)

        self.history.append(mode_now)
        if len(self.history) > self.smooth_frames:
            self.history.pop(0)

        vals, counts = np.unique(self.history, return_counts=True)
        best_idx = int(vals[np.argmax(counts)])
        freq = counts.max() / len(self.history)

        if freq >= self.conf_thresh:
            self.current_mode = best_idx
...
def main():
    ...
    # OPTIMIZATION: Pre-calculate normalization constants on GPU
    # Reshaped to (1, 3, 1, 1) for broadcasting
    mean_tensor = torch.tensor(DEFAULT_MEAN, device=device).view(1, 3, 1, 1)
    std_tensor = torch.tensor(DEFAULT_STD, device=device).view(1, 3, 1, 1)
    ...
    mode_detector = LeftModeDetector()
    current_mode = 0
                    ...
                    for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
                        ...
                        if label_text == "Right":
                            ...
                            if crop_img is not None:
                                ...
                                # OPTIMIZATION: Defer normalization to GPU
                                # Append raw uint8 (H, W, 3)
                                right_images.append(crop_img)
                                right_meta.append(
                                    {
                                        "center": [c_x, c_y],
                                        "scale": raw_s,
                            ...
                            # LEFT HAND: used only for mode / gesture
                            left_landmarks_list.append(hand_landmarks)

                    current_mode = mode_detector.update(left_landmarks_list)

                # --- RUN HAMBA ON RIGHT HANDS ---
                if len(right_images) > 0:
                    # OPTIMIZATION: Batch move to GPU as uint8 -> Float -> Normalize on GPU
                    # 1. Stack raw images (B, H, W, 3)
                    batch_uint8 = np.stack(right_images)
                    
                    # 2. Move to GPU (Non-blocking)
                    batch_tensor = torch.from_numpy(batch_uint8).to(device, non_blocking=True)
                    
                    # 3. Permute to (B, C, H, W) and Normalize
                    batch_tensor = batch_tensor.permute(0, 3, 1, 2).float().div_(255.0)
                    tensor_batch = (batch_tensor - mean_tensor) / std_tensor
