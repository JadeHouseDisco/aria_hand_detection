...
class OneEuroFilter:
    ...
    def __call__(self, x, t=None):
        ...
        # Safety check for huge jumps (prevents smoothing artifacts on teleportation)
        if np.linalg.norm(x - self.x_prev) > 0.5: # If jump > 50cm
            self.x_prev = x # Reset filter
            return x
...
def main():
    ...
    # Increased Beta slightly for better responsiveness on rotations
    filters_skeleton = {"Left": OneEuroFilter(beta=0.1), "Right": OneEuroFilter(beta=0.1)}
    ...
    print("\n=== TELEOP V16 (NO FLIP / RAW PHYSICS) ===")
    ...
    while True:
        ...
        # [FIX] NO FLIP. Feed raw camera data.
        ...
        if results.multi_hand_landmarks:
            for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
                ...
                # [FIX] MediaPipe logic for RAW (non-mirrored) images
                # In raw video, if you raise Right hand, it appears on the Left side of screen.
                # MediaPipe usually calls this "Left".
                # We need to map: MP "Left" -> User "Right".
                if mp_label == "Left":
                    label_text = "Right"
                    is_right_flag = 1
                else:
        ...
        cv2.imshow('Hamba Teleop V16 (RAW)', frame)
