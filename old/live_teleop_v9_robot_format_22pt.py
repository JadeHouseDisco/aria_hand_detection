...
def synthesize_forearm(skeleton_21):
    """
    Takes (21, 3) array.
    Calculates forearm point = Wrist + (Wrist - Middle_Base).
    Returns (22, 3) array with forearm at index 0.
    """
    wrist = skeleton_21[0]       # Index 0
    middle_base = skeleton_21[9] # Index 9
    
    # Vector calculation: Extend backwards from wrist
    forearm_point = wrist + (wrist - middle_base)
    
    # Prepend to make it index 0
    # Reshape forearm to (1, 3) for stacking
...
def main():
    ...
    while True:
        ...
        if len(batch_images) > 0:
            ...
            # 1. Get Raw 21x3 Points
            keypoints_3d = out['pred_keypoints_3d'].detach().cpu().numpy() # (N, 21, 3)
            ...
            for i in range(len(batch_images)):
                ...
                # 2. Filter the 21 points (Smooth them)
                flat_21 = raw_21.flatten()
                smoothed_flat = filters[label](flat_21)
                smoothed_21 = smoothed_flat.reshape(21, 3)
                
                # 3. Synthesize 22nd point (Forearm)
                # This function adds the point at Index 0
                robot_skeleton = synthesize_forearm(smoothed_21)
                
                # 4. Final Data ready for Robot
                # robot_skeleton.shape is now (22, 3)
                print(f"[{label}: Ready {robot_skeleton.shape}] ", end="")
                
                color = (0, 255, 0) if label == "Right" else (0, 0, 255)
                cv2.rectangle(frame, (box[0], box[1]), (box[2], box[3]), color, 2)
                cv2.putText(frame, f"{label} (22pts)", (box[0], box[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)

        else:
            print(f"\rFPS: {1 / (time.time() - t0):.0f} | Searching...", end="")

        cv2.imshow('Hamba Teleop V9', frame)
