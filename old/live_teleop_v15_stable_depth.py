...
# [FIX] New Scaling Factor for Palm-based cropping
# Wrist-to-Knuckle is much smaller than full hand, so we need a larger multiplier 
# to ensure the fingers stay in frame. 4.0 to 5.0 is usually safe.
PALM_SCALE_MULTIPLIER = 4.5 
...
class Visualizer3D:
    ...
    def update(self, hands_dict):
        ...
        self.ax.scatter([0], [0], [0], c='blue', marker='^', s=50) # Camera
        ...
        for label, skeleton in hands_dict.items():
            ...
            # Mapping: PlotX=DataX, PlotY=DataZ(Depth), PlotZ=DataY(Up/Down)
...
def get_stable_crop_params(landmarks, img_shape):
    """
    [FIX] Calculates crop based on RIGID Palm Distance (Index 0 to 9)
    instead of the flexible bounding box.
    """
    img_h, img_w = img_shape[:2]
    
    # Extract key landmarks (in pixels)
    wrist = np.array([landmarks[0].x * img_w, landmarks[0].y * img_h])
    middle_mcp = np.array([landmarks[9].x * img_w, landmarks[9].y * img_h])
    
    # 1. Calculate Palm Scale (Euclidean Distance)
    palm_size = np.linalg.norm(wrist - middle_mcp)
    
    # 2. Determine Crop Size (Fixed multiplier of palm)
    s = palm_size * PALM_SCALE_MULTIPLIER
    
    # 3. Determine Center (Center on Middle Knuckle for stability)
    # Using Middle MCP (Index 9) is often more stable than the wrist for framing fingers
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
                ...
                # [FIX] Use Stable Palm-Based Cropping
                # We pass the FULL landmark object to calculate wrist-to-knuckle distance
                crop_params = get_stable_crop_params(hand_landmarks.landmark, img_rgb.shape)
                crop_img = crop_image(img_rgb, crop_params)
                
                # Extract metadata from new stable calculator
                # x1, y1, x2, y2, pad_l, pad_t, pad_r, pad_b, SCALE, CENTER
                s = crop_params[8]
                ...
                if crop_img is not None:
                    ...
                    visual_info.append({
                        'label': label_text,
                        'center': [c_x, c_y], # Stable Center
                        'scale': s,           # Stable Scale
                        'img_size': [h, w],
                        'crop_rect': crop_params[0:4] # For drawing box on screen
                    })
        ...
        if len(batch_images) > 0:
            ...
            for i in range(len(batch_images)):
                ...
                smooth_trans = filters_pose_trans[label](cam_t) # Filter the translation
                raw_21_cam_space = keypoints_3d_crop[i] + cam_t # Apply translation to relative skeleton (Important: apply BEFORE filtering skeleton)
                
                # Filter the absolute skeleton (smoothing position + posture together)
                ...
                # Draw the STABLE crop box (to prove it doesn't shrink)
                x1, y1, x2, y2 = info['crop_rect']
                cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
                cv2.putText(frame, f"{label} Stable", (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
        ...
        cv2.imshow('Hamba Teleop V15', frame)
