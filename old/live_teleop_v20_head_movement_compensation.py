...
# --- 3D VISUALIZER (FIRST PERSON VIEW) ---
class Visualizer3D:
    def __init__(self):
        plt.ion()
        ...
        self.ax.view_init(elev=20, azim=180)
        ...
        while True:
            ...
            if len(batch_images) > 0:
                ...
                for i in range(len(batch_images)):
                    ...
                    points_target_cam_space = np.stack([
                        points_cam[:, 2],       # Target X = MP Z
                    -points_cam[:, 0],       # Target Y = -MP X
                    -points_cam[:, 1]        # Target Z = -MP Y
                    ], axis=1)

                    points_world = (R_head @ points_target_cam_space.T).T
