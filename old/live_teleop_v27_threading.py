...
class AsyncHeadPose:
    def __init__(self, aria_observer, head_estimator):
        self.observer = aria_observer
        self.estimator = head_estimator
        
        self.running = False
        self.thread = None
        self.lock = threading.Lock()
        
        # Store the latest calculated matrix here
        self._latest_matrix = np.eye(3)

    def start(self):
        self.running = True
        # daemon=True means this thread dies automatically if the main program crashes
        self.thread = threading.Thread(target=self._worker_loop, daemon=True)
        self.thread.start()
        print("Async IMU Thread Started.")
    ...
    def get_matrix(self):
        """
        Main thread calls this to get the instantaneous rotation.
        Extremely fast (just a memory copy).
        """
        with self.lock:
            return self._latest_matrix.copy()

    def _worker_loop(self):
        """
        The 'Mosquito' loop. Runs fast, does little work.
        """
        while self.running:
            # 1. Grab all pending IMU packets
            imu_batch = self.observer.pop_imu_queue()

            if imu_batch:
                # 2. Process them all to catch up history
                for (accel, gyro, ts) in imu_batch:
                    
                    # --- REMAP AXES (Aria Right IMU -> Estimator) ---
                    # We do the math HERE, so the main thread doesn't have to.
                    
                    # Aria: X=Down, Y=Right, Z=Back
                    # Est:  X=Fwd,  Y=Left,  Z=Up
                    
                    # Map Accel
                    accel_mapped = np.array([
                        -accel[2],  # Est X = -Aria Z
                        -accel[1],  # Est Y = -Aria Y
                        -accel[0]   # Est Z = -Aria X (Gravity)
                    ])
                    ...
                    # Update the internal state of the estimator
                    self.estimator.update(accel_mapped, gyro_mapped, ts)
                
                # 3. Update the output matrix
                # We calculate this once per batch of packets
                new_mat = self.estimator.get_rotation_matrix()
                
                with self.lock:
                    self._latest_matrix = new_mat
            
            # 4. Sleep to prevent CPU burning
            # 0.001 = 1ms. Checks 1000 times a second.
            # This releases the Python GIL so the main thread can run PyTorch.
            time.sleep(0.001)
...
class AriaImageListener:
    ...
    def pop_imu_queue(self):
        """
        Thread-safe method to get AND CLEAR only the IMU data.
        """
        with self.lock:
...
def main():
    ...
    async_imu = AsyncHeadPose(aria_observer, head_pose_estimator)
    async_imu.start()
    ...
    try:
        while True:
            raw_img, _ = aria_observer.get_data_queue()
            ...
            if len(batch_images) > 0:
                ...
                j_regressor = model.mano.J_regressor # Shape: (16, 778)
                ...
                # --- START: SYNTHESIZE FOREARM (Vectorized) ---
                
                # 1. Extract joints (Keep shape as B, 3)
                wrists = joints_3d[:, 0, :]
                middle_mcps = joints_3d[:, 9, :]

                # 2. Calculate the vector
                # This works for both Left and Right hands automatically 
                # because we already did the flipping/correction above.
                forearm_points = wrists + (wrists - middle_mcps)

                # 3. Reshape to (Batch, 1, 3) to allow concatenation
                forearm_points = forearm_points.unsqueeze(1)

                # 4. Concatenate to the BEGINNING of the skeleton
                # New Shape: (Batch, 22, 3) where Index 0 is now Forearm
                joints_3d = torch.cat([forearm_points, joints_3d], dim=1)

                # --- END SYNTHESIZE FOREARM ---
                ...
                for i in range(len(batch_images)):
                    ...
                    current_skeleton = batch_skeleton_np[i]   # Shape (22, 3)
