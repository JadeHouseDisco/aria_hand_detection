...
def main():
    ...
    while True:
        ...
        if len(batch_images) > 0:
            ...
            poses = out['pred_mano_params']['hand_pose'].cpu().numpy()
            fps = 1 / (time.time() - t0)
            ...
            for i in range(len(batch_images)):
                ...
                box = visual_info[i][0]
                raw_pose = poses[i] # (15, 3, 3)
                
                # --- FILTERING STEP ---
                # 1. Flatten to 1D array for the filter (135 numbers)
                flat_pose = raw_pose.flatten()
                # 2. Pass through the specific filter for this hand
                smoothed_pose_flat = filters[label](flat_pose)
                # 3. Reshape back to Matrix (optional, if retargeter needs matrix)
                smoothed_pose = smoothed_pose_flat.reshape(15, 3, 3)
                
                # Print sanity check (Mean of raw vs smoothed)
                diff = np.abs(flat_pose - smoothed_pose_flat).mean()
                print(f"[{label}: Jitter Rem={diff:.4f}] ", end="")
