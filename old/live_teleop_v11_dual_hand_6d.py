...
def main():
    ...
    filters_pose_rot = {"Left": OneEuroFilter(beta=0.01), "Right": OneEuroFilter(beta=0.01)}
    ...
    while True:
        ...
        if len(batch_images) > 0:
            ...
            for i in range(len(batch_images)):
                ...
                full_cam_t = cam_crop_to_full(pred_cam[i:i+1], box_center_t, box_size_t, img_size_t, FOCAL_LENGTH).detach().cpu().numpy()[0]
                
                smooth_trans = filters_pose_trans[label](full_cam_t)
                raw_rot_mat = wrist_rot_matrix[i][0]
                ...
                # Build 4x4 (This is what you send to robot)
                wrist_4x4 = np.eye(4)
                ...
                # --- PRINT ---
                # Print Translation X, Y, Z for each hand detected
                t_str = f"[{smooth_trans[0]:.2f}, {smooth_trans[1]:.2f}, {smooth_trans[2]:.2f}]"
                print(f"{label}: {t_str} ", end="")
                
                # Draw
                color = (0, 255, 0) if label == "Right" else (0, 0, 255)
                bbox = info['bbox']
                cv2.rectangle(frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), color, 2)
                cv2.putText(frame, f"{label} (22pts)", (bbox[0], bbox[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)

        else:
            print(f"\rFPS: {1 / (time.time() - t0):.0f} | Searching...", end="")

        cv2.imshow('Hamba Teleop V11', frame)
