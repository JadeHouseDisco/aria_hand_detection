...
class HeadPoseEstimator:
    ...
    def remap_axes(self, accel, gyro):
        ...
        # Swap axes to align with Visualizer Coordinate System
        ax = -raw_ay
        ay = -raw_az
        az = raw_ax
        gx = -raw_gy
    ...
    def get_rotation_matrix(self):
        ...
        R_y = np.array([[c_r, 0, s_r], [0, 1, 0], [-s_r, 0, c_r]])
...
class AriaImageListener:
    ...
    def on_image_received(self, image, record):
        if record.camera_id == aria_sdk.CameraId.Rgb:
            with self.lock:
                self.rgb_image = np.rot90(image, -1).copy()
...
def setup_aria_stream(ip_address: str, profile_name: str):
    ...
    # Tighter queue for lower latency
...
def main():
    ...
    try:
        while True:
            # 1. Get Data
            # raw_img IS RGB directly from Aria SDK (we removed the BGR conversion)
            ...
            # 2. Process Head Rotation
            R_head = pose_estimator.process(accel, gyro, ts_imu)

            # 3. Detect Hands (On RGB image directly)
            # No copy needed if we don't plan to modify it for OpenCV display
            results = hands.process(raw_img)
            ...
            if results.multi_hand_landmarks:
                for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
                    ...
                    raw_s, c_x, c_y = get_stable_crop_params(hand_landmarks.landmark, raw_img.shape)
                    smooth_s = filters_scale[label_text](raw_s)
                    crop_rect_params = compute_crop_coords(c_x, c_y, smooth_s, raw_img.shape)
                    
                    # Crop using the RGB image
                    crop_img = crop_image(raw_img, crop_rect_params)
                    ...
                    if crop_img is not None:
                        ...
                        # We keep visual info only for rotation calc, not drawing
                        visual_info.append({
                            "label": label_text,
                            "center": [c_x, c_y],
                            "scale": smooth_s,
                            "img_size": raw_img.shape[:2],
                        })
            ...
            if len(batch_images) > 0:
                ...
                for i in range(len(batch_images)):
                    ...
                    filters_pose_trans[label](cam_t)
            ...
            # 6. Performance Logging (Every 100 frames)
            frame_count += 1
            if frame_count % 100 == 0:
                fps = frame_count / (time.time() - start_time)
                print(f"Running... Current FPS: {fps:.1f}")
