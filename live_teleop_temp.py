...
class Visualizer3D:
    ...
    def update(self, hands_dict):
        ...
        for label, skeleton in hands_dict.items():
            ...
            # Label the Wrist (Index 0)
            self.ax.text(xs[0], ys[0], zs[0], label, fontsize=8)
        ...
        with torch.no_grad():
            while True:
                ...
                if len(right_images) > 0:
                    ...
                    if DISPLAY:
                        # --- VISUALIZATION: MESH PROJECTION ---
                        # Project vertices to image
                        verts_cam = pred_verts + wrist_position.unsqueeze(1) # (B, 778, 3)
                        
                        fx = FOCAL_LENGTH
                        fy = FOCAL_LENGTH
                        cx_img = TARGET_WIDTH / 2.0
                        cy_img = TARGET_HEIGHT / 2.0
                        
                        x_p = (verts_cam[..., 0] / verts_cam[..., 2]) * fx + cx_img
                        y_p = (verts_cam[..., 1] / verts_cam[..., 2]) * fy + cy_img
                        
                        pts = torch.stack([x_p, y_p], dim=-1).cpu().numpy().astype(np.int32)
                        
                        for b in range(pts.shape[0]):
                            valid_mask = (pts[b, :, 0] >= 0) & (pts[b, :, 0] < TARGET_WIDTH) & \
                                         (pts[b, :, 1] >= 0) & (pts[b, :, 1] < TARGET_HEIGHT)
                            valid_pts = pts[b][valid_mask]
                            
                            # Draw points (Green)
                            debug_frame[valid_pts[:, 1], valid_pts[:, 0]] = [0, 255, 0]
                            # Make them slightly visible (cross or 2x2)
                            debug_frame[valid_pts[:, 1], np.clip(valid_pts[:, 0]+1, 0, TARGET_WIDTH-1)] = [0, 255, 0]
